  ...
  return d3
    ...
    .map(([commit, lines]) => {
      ...
      let ret = {
        ...
        daysWorked: new Set(lines
    .map(l => {
      const dt = l.datetime instanceof Date ? l.datetime : new Date(l.datetime);
      return isNaN(dt) ? null : dt.toISOString().split('T')[0]; // YYYY-MM-DD
    })
    .filter(Boolean)
).size
      };
...
function renderCommitInfo(data, commits) {
  ...
  // Add total LOC
  dl.append('dt').html('<abbr title="Lines of code">LOC</abbr>');
  ...
  //add average depth
  const depth = d3.max(data, d => d.depth)
  dl.append('dt').text('Avg Depth');
  dl.append('dd').text(depth);

  //average file lengths
  ...
  dl.append('dd').text(d3.sum(commits, c => c.daysWorked));
...
function renderTooltipContent(commit) {
  ...
  author.textContent = commit.author || 'N/A';
